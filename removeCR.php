...
//The PHP for the form input
  if(isset($_POST['submitBtn']))
  {
    ...
    $SQLInput = "CALL deleteCRPermsUsingID(\"{$CRPRID}\")";
    $queryOutput = $conn->query($SQLInput);
    $conn -> close();
    //Delete Permissions

    //Delete CRPR Connection
    //Connect to server
    $conn = new mysqli("oaicontezoagile.mysql.db","oaicontezoagile","M5fgq184HDVu","oaicontezoagile");
    if ($conn -> connect_errno) {
      echo "<br>Failed to connect to MySQL: " . $conn -> connect_error;
    ...
    $SQLInput = "CALL removeCRfromPR(\"{$CRPRID}\")";
    $queryOutput = $conn->query($SQLInput);
    $conn -> close();
    //Delete CRPR Connection
  ...
  <div class="container">
    <div class="row">
      ...
      <div id="loginCol" class="col-md-4 col-xs-10">
        <h1>UoD Remove Co-Researcher</h1> <hr>
        <form action="" method="POST">
          <div class="form-group">
            <label for="username">Select the Co-Researcher remove:</label>
            <br>
            <?php
              //Connect to server
              $conn = new mysqli("oaicontezoagile.mysql.db","oaicontezoagile","M5fgq184HDVu","oaicontezoagile");
              ...
              if($queryOutput->num_rows > 0){
                while($row = $queryOutput->fetch_object()){
                  echo "<input type=\"radio\" id=\"{$row -> CRUsername}\" value=\"{$row -> CRUsername}\" name = \"username\"> <label for=\"{$row -> CRUsername}\">{$row -> CRUsername}</label><br>";
                }
              }else{
             ...
             ?>
          <br>
          <button id="submitBtn"type="submit" class="btn btn-primary" name ="submitBtn">Remove Co-Researcher</button>
        </form>
      </div>
      <div class="col-md-4 col-xs-1">
